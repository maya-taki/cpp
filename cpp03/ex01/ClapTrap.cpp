...
/*   Updated: 2026/09/16 18:44:05 by mtakiyos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ClapTrap::takeDamage(unsigned int amount)
{
	if (_hit_points == 0)
	{
		std::cout << _name << " takes " << amount << " damage but it is already dead ):" << std::endl;
		return ;
	}
	...
	if (_hit_points)
	{
		...
		_hit_points -= amount;
		//if (_hit_points >= amount)
		//	std::cout << "[ " << _name << " has " << _hit_points << " health points left ]" <<std::endl;
...
void	ClapTrap::beRepaired(unsigned int amount)
{
	...
	if (!_energy_points)
		std::cout << _name << " ran out of energy. Can't be repaired" << std::endl;
	else if (!_hit_points)
